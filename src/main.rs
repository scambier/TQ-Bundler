    ...
    let matches = App::new("TIC-80 Bundler")
        ...
        .subcommand(
            ...
            .arg(
                Arg::new("POST_BUILD")
                    .long("post-build")
                    .help("Shell command to run after bundling. Supports {input} and {output} placeholders.")
                    .takes_value(true)
                    .required(false),
            )
            .arg(
                Arg::new("POST_OUTPUT")
                    .long("post-output")
                    .help("Output file generated by --post-build. Used by TIC-80 runtime.")
                    .takes_value(true)
                    .required(false),
            )
            .arg(
...
fn run_post_build_step(config: &Config) -> bool {
    ...
    let status = Command::new("cmd").args(["/C", resolved.as_str()]).status();

    #[cfg(not(target_os = "windows"))]
    let status = Command::new("sh").args(["-c", resolved.as_str()]).status();
...
pub fn compile_pipeline(config: &Config) -> bool {
    let compiled = compile(config);
    if !compiled {
        return false;
    }
    run_post_build_step(config)
}
...
fn run(matches: &ArgMatches) {
    ...
    let compiled = compile_pipeline(&config);
    ...
    if let Some(tic_path) = tic_path {
        let output_path = config
            .base_folder
            .join(config.runtime_output_file())
            .to_str()
            .unwrap()
            .to_string();
